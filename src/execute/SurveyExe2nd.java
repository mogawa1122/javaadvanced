...
import common.ContentFromClass;
import common.DisplayChoice;
import common.Extraction;
import common.InputCheck;

public class SurveyExe2nd {
	...
	public static void main(String[] args) {
		...
		Content cfc = new ContentFromClass();
//		ContentFromClass cfc = new ContentFromClass();
		...
		while (true) {
			...
			// 次リスト番号のデータを取得
			sentence = cfc.getContent();
			
			// textタグのコンテンツを取得
			content = ext.extContent(sentence.get(i), "{"+cfc.stText+"}", "{/"+cfc.stText+"}");	
			System.out.println(content);
			answer = content;
			
			// EOFタグがあればアンケート終了
			if (sentence.contains(cfc.stEOF)) {
				break;
			}
		
			// nextタグのコンテンツを取得
			nextString = ext.extContent(sentence.get(0), "{"+cfc.stNext+"}", "{/"+cfc.stNext+"}");
			
			// singleChoiceJump
			if (sentence.contains(cfc.stSCJ)) {
				
				// singleChoiceJumpの選択肢部を取得
	    		String contentSCJ = ext.extContent(sentence.get(0), "{" + cfc.stSCJ + "}\n", "{/" + cfc.stSCJ + "}");
			...
			// multipleChoice
			} else if (sentence.contains(cfc.stMC)) {
	    		...
	    		String contentMC = ext.extContent(sentence.get(0), "{" + cfc.stMC + "}\n", "{/" + cfc.stMC + "}");
    	        ...
    	        for (String splitAns : splitAnswers) {
    	    		// 回答文を作成
        	        notDuplicate = dupCheck.add(splitAns);
        	        if (notDuplicate) {
        	        	answer = answer + "【回答】 "   
